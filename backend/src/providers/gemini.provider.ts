...
export class GeminiProvider {
  ...
  private async callAgent(prompt: string, context: string, retries = 3): Promise<string> {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        logger.warn(`Gemini API call failed (attempt ${i + 1}/${retries})`, { error });
  ...
  async generateInsights(scrapedData: ScrapedData, lead: LeadFormData): Promise<AIInsights> {
    try {
      logger.info('Generating insights with Gemini single-call pipeline');
...
Make everything specific to THIS company, not generic.`;
      ...
      logger.info('Calling Gemini API with comprehensive prompt...');
      ...
      logger.info('Gemini insights generation completed (single call)');
      return insights;

    } catch (error) {
      logger.error('Failed to generate insights with Gemini', { error });
